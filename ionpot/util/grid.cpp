...
namespace ionpot::util {
	...
	{
		IONPOT_INT_NATURAL(row);
		IONPOT_INT_NATURAL(column);
	}
	...
	{
		IONPOT_INT_POSITIVE(rows);
		IONPOT_INT_POSITIVE(columns);
	}
	...
	{
		size.assert_valid();
		IONPOT_INT_WITHIN(size.rows, rows);
		IONPOT_INT_WITHIN(size.columns, columns);
	}

	void
	GridSize::assert_valid(GridIndex i) const
	{
		IONPOT_INT_INDEX(i.row, rows);
		IONPOT_INT_INDEX(i.column, columns);
	}
